...
| US ID | US Title | Task ID | Task Title | Description | Est. (h) | Assigned To | Status |
|-------|----------|---------|------------|-------------|----------|-------------|--------|
| US46 | View Daily Nutritional Overview in Dashboard | T01 | Scaffold frontend application with DDD folder structure and router | Initialize the Vue + Vite project following the bounded-context folder structure (`nutrition-tracking/`, `body-metrics/`, `analytics/`, `smart-scan/`, `recommendations/`, `shared/`). Configure Vue Router with public and protected route groups, integrate PrimeVue with Material theme, set up Pinia, configure Axios `BaseApi` and `BaseEndpoint` following the Learning Center pattern, and configure `json-server` with `db.json` fixtures for all Sprint 2 endpoints. | 5 | Villarreal Bazan, Angel Martin | Done |
...
| US25 | Body Fat and Lean Mass Monitoring | T26 | Build body composition view for muscle-gain segment | Implement the `/metrics/composition` view, visible to users with a muscle-gain goal, displaying estimated body fat percentage and lean body mass in kg calculated with the Navy formula. Provide PrimeVue InputNumber fields for waist and neck circumferences. On save, call `useBodyMetricsStore.updateBodyComposition()` mock. Display a PrimeVue Message alert when body fat increase exceeds the recommended lean bulk ratio. | 4 | Espinoza Cruz, Angela Milagros | Done |
| US26 | Log Height Update | T27 | Add height update field to body metrics form | Add a PrimeVue InputNumber height field (cm) to the `/metrics/weight` view alongside the weight entry form. On save, call `useBodyMetricsStore.updateHeight()` mock and trigger recalculation of BMI, BMR, and TDEE in the health metrics summary reactively. | 1 | Espinoza Cruz, Angela Milagros | Done |
| US41 | Receive Climate-Based Food Recommendations | T28 | Define DDD layer files for Recommendations bounded context | Create the `Recommendation` and `WeatherContext` domain entities, the `RecommendationsApi` infrastructure class extending `BaseApi` with `getClimateRecommendations()`, `activateTravelMode()`, and `getPantryRecipes()` methods, the `RecommendationsAssembler`, and the `useRecommendationsStore` Pinia store with all corresponding actions following the Learning Center DDD pattern. | 3 | Vergaray Calderon, Rose Almendra | Done |
| US41 | Receive Climate-Based Food Recommendations | T29 | Build climate-based food recommendations view | Implement the `/recommendations` view displaying a list of food recommendation cards adapted to the current weather at the user's location. Each card shows the food name, description, and reason for recommendation based on weather context. Weather and location data sourced from `useRecommendationsStore` mock. Apply `aria-live="polite"` to the recommendations list region. | 4 | Vergaray Calderon, Rose Almendra | Done |
| US42 | Activate Travel Mode for Local Food Recommendations | T30 | Build travel mode activation panel and local recommendations view | Implement the travel mode toggle within the `/recommendations` view using a PrimeVue ToggleButton. When activated, display a PrimeVue InputText for manual city/country entry alongside an auto-detect option. Display local dish recommendation cards compatible with the user's nutritional profile. On activate, call `useRecommendationsStore.activateTravelMode()` mock. | 4 | Vergaray Calderon, Rose Almendra | Done |
| US43 | Pantry-Based Recipe Suggestions | T31 | Define DDD layer files for Pantry bounded context | Create the `PantryItem` and `Recipe` domain entities, the `PantryApi` infrastructure class extending `BaseApi` with `getPantryItems()`, `addPantryItem()`, `deletePantryItem()`, and `getRecipeSuggestions()` methods, the `PantryAssembler`, and the `usePantryStore` Pinia store with all corresponding actions. | 3 | Vergaray Calderon, Rose Almendra | Done |
| US43 | Pantry-Based Recipe Suggestions | T32 | Build pantry management view with ingredient list | Implement the `/pantry` view displaying the user's registered pantry items as a PrimeVue DataTable with ingredient name, quantity, and unit. Provide an add form with PrimeVue InputText for ingredient name and PrimeVue InputNumber for quantity. Each row has a delete action with PrimeVue ConfirmDialog. On add/delete, call `usePantryStore` mock actions reactively. | 4 | Vergaray Calderon, Rose Almendra | Done |
| US43 | Pantry-Based Recipe Suggestions | T33 | Build recipe suggestions view based on pantry contents | Implement the recipe suggestions section within `/pantry` displaying recipe cards generated from current pantry contents. Each card shows recipe name, required ingredients, estimated calories, and macro breakdown filtered by the user's nutritional profile and dietary restrictions. Data sourced from `usePantryStore.getRecipeSuggestions()` mock. | 3 | Vergaray Calderon, Rose Almendra | Done |
| — | Cross-cutting | T34 | Integrate i18n module into the web application and apply to all Sprint 2 views | Port the i18n dictionaries (en_US / es_419) from the landing page to the Vue application following the Learning Center `i18n.js` pattern. Add `en.json` and `es.json` entries for all static text in all Sprint 2 views. Wire the language selector in the app Layout component. Persist language preference in `localStorage`. | 3 | Vergaray Calderon, Rose Almendra | Done |
| US37 | Scan a Food Plate Photo | T35 | Define DDD layer files for Smart Scan bounded context | Create the `ScanResult` and `ScannedMeal` domain entities, the `SmartScanApi` infrastructure class extending `BaseApi` with `scanFoodPlate()`, `scanMenuPhoto()`, `confirmScanResult()`, and `enforceRestrictions()` methods, the `SmartScanAssembler`, and the `useSmartScanStore` Pinia store with all corresponding actions following the Learning Center DDD pattern. | 3 | Del Aguila Del Aguila, Olenka Priscilla | Done |
| US37 | Scan a Food Plate Photo | T36 | Build food plate scan view with photo upload and result preview | Implement the `/smart-scan/plate` view allowing the user to upload or capture a photo of their food plate using a PrimeVue FileUpload component. On upload, call `useSmartScanStore.scanFoodPlate()` mock and display the scan result preview card showing estimated food items, calories, and macros. Provide a confirm button to log the result to the daily nutrition log. | 5 | Del Aguila Del Aguila, Olenka Priscilla | Done |
| US37 | Scan a Food Plate Photo | T37 | Build scan result confirmation and manual correction form | Implement the scan result editing panel within `/smart-scan/plate` allowing the user to adjust identified food items, quantities, and serving sizes before confirming. On confirm, call `useSmartScanStore.confirmScanResult()` mock and add the entry to the nutrition log via `useNutritionStore`. | 3 | Del Aguila Del Aguila, Olenka Priscilla | Done |
| US38 | Menu Recommendations for Weight-Loss Segment | T38 | Build restaurant menu scan view for weight-loss segment | Implement the `/smart-scan/menu` view allowing the user to upload a photo of a restaurant menu. On upload, call `useSmartScanStore.scanMenuPhoto()` mock and display a filtered list of menu options recommended for caloric deficit, each with estimated calories and a deficit-compatibility indicator. | 4 | Del Aguila Del Aguila, Olenka Priscilla | Done |
| US39 | Menu Recommendations for Muscle-Gain Segment | T39 | Build restaurant menu scan view for muscle-gain segment | Extend the `/smart-scan/menu` view to display high-protein menu options for users with a muscle-gain goal. Show each option with estimated protein content and a protein-target compatibility indicator alongside the caloric information. | 3 | Del Aguila Del Aguila, Olenka Priscilla | Done |
| US40 | Enforcement of Dietary Restrictions in Smart Scan | T40 | Build dietary restriction enforcement layer in Smart Scan results | Implement the restriction enforcement logic within the scan result display. Flag or exclude items containing ingredients that match the user's configured dietary restrictions. Display a PrimeVue Message warning for each flagged item explaining the restriction conflict. Data sourced from `useSmartScanStore` and user restriction mock data. | 3 | Del Aguila Del Aguila, Olenka Priscilla | Done |
