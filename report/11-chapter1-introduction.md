...
How might we help users make informed food decisions in any context without requiring significant effort or nutritional expertise from them?
...
We believe that young adults seeking muscle gain will maintain their macro tracking discipline during travel periods if they use the Travel Mode feature, which suggests healthy local dishes compatible with their active nutritional profile. <br>We will know our solution is working when platform usage data shows that the drop in daily log completion rate during travel periods among this segment decreases by at least 40% compared to their pre-NutriSense baseline, measured through in-app tracking of log entries during days when Travel Mode is active. This measurement approach is grounded in established Health research practice, as Olaniyi et al. (2022) demonstrate that behavioral logs generated by health apps can be transformed into daily time series that accurately characterize each user's real activity patterns, enabling reliable before-and-after comparisons without relying on self-reported data.
